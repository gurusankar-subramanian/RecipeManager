...
public class ReceipeController {
	...
	public ResponseEntity<?> creatTask(@RequestBody ReceipeModel receipe) {
		if(receipe.getIncredients().size()> 0 && receipe.getIncredients().size() <= 4){
			receipe = receipeService.save(receipe);
			return new ResponseEntity<ReceipeModel>(receipe, HttpStatus.CREATED);
		}else{
	...
	public ResponseEntity<?> getById(@PathVariable("id") Long id) {
		ReceipeModel receipe= receipeService.find(id);
		if(!Objects.isNull(receipe))
			return new ResponseEntity<ReceipeModel>(receipe, HttpStatus.OK);
		else
